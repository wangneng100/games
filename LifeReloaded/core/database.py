...
class Database:
    def __init__(self, cfg_path='.env', time_out=3600, debug=False) -> None:
        ...
        redis_config = config.get('REDIS', 'localhost')
        
        # If Redis is 'localhost' or empty, connect without password
        if redis_config in ['localhost', 'YOUR_PASSWORD', '', None]:
            self.client = DatabaseBackend(host='localhost',
                                          port=6379,
                                          db=0)
        else:
            # Use as password for remote Redis
            self.client = DatabaseBackend(host='localhost',
                                          port=6379,
                                          db=0,
                                          password=redis_config)
